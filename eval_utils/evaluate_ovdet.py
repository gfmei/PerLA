...
def evaluate(
    ...
    for curr_iter, batch_data_label in enumerate(dataset_loader):
        ...
        caption_box_coords = caption_box_coords.reshape(batch_size, nqueries, 7)
        ...
        caption_box_angle = caption_box_coords[..., -1].reshape(batch_size, nqueries)
        ...
        box_corners[caption_box_mask == 1] = caption_box_corners[caption_box_mask == 1]
        ...
        for batch_id, scene_id in enumerate(batch_data_label["scan_idx"].cpu().tolist()):
            scene_name = scene_list[scene_id]
            scene_prediction = []
            scene_retain_results = good_bbox_masks[batch_id].tolist() # nqueries
            for prop_id in range(nqueries):
                ...
                scene_prediction.append(
                    dict(
                        bbox=outputs["box_corners"][batch_id, prop_id].cpu().tolist(),
                        caption=captions[batch_id][prop_id],
                        match_iou=match_box_ious[batch_id, prop_id].cpu().tolist()
                    )
                )
            dump_dir = os.path.join(args.checkpoint_dir, f'{task_name}_viz_ovdet')
            os.makedirs(dump_dir, exist_ok=True)
            with open(os.path.join(dump_dir, scene_name + '.json'), 'w') as file:
    ...
    if is_primary():
        ...
        with open(os.path.join(args.checkpoint_dir, f"{task_name}_corpus_val.json"), "w") as f: 
            json.dump(corpus, f, indent=4)
        
        with open(os.path.join(args.checkpoint_dir, f"{task_name}_pred_val.json"), "w") as f:
            json.dump(candidates, f, indent=4)
        
        with open(os.path.join(args.checkpoint_dir, f"{task_name}_pred_gt_val.json"), "w") as f:
            pred_gt_val = {}
