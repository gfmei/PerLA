...
def evaluate(
    ...
    descriptions = dataset_loader.dataset.descriptions
    scan_names = dataset_loader.dataset.scan_names
    task_name = dataset_loader.dataset.task_name
    corpus = {}
    for anno in descriptions:
        scene_id = anno['scene_id']
        corpus[scene_id] = corpus.get(scene_id, []) + [anno['answers'][0].lower()]
    ...
    for curr_iter, batch_data_label in enumerate(dataset_loader):
        ...
        for idx in range(output_ids.shape[0]):
            scene_id = scan_names[sample_index[idx]]
            key = scene_id
            answer = answers[idx]
