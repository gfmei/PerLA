...
from models.common import pytorch_utils as pt_utils, pointnet2_utils
...
class _PointnetSAModuleBase(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor, features: torch.Tensor = None) -> (torch.Tensor, torch.Tensor):
        ...
        new_xyz = index_points(
            xyz, farthest_point_sample(xyz, self.npoint)).contiguous() if self.npoint is not None else None
...
class PointnetSAModuleVotes(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        new_xyz = index_points(xyz, inds) if self.npoint is not None else None
