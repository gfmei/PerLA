...
from models.common.criterion import build_criterion
...
from models.common.transformer import (
    MaskedTransformerEncoder, TransformerDecoder,
    TransformerDecoderLayer, TransformerEncoder,
    TransformerEncoderLayer
)
from models.common.vote_query import VoteQuery
from models.common.serialization import divide_point_cloud_curve
from models.common.pointnet2_modules import PointnetSAModuleVotes
from libs.pc_utils import index_points, scale_points, shift_scale_points
...
class ModelVote2capDetr(nn.Module):
    ...
    def forward(self, inputs, n_splits=-1, is_eval: bool = False, order='xyz'):
        ...
        # # neigh_xyz = index_points(p_xyz, neigh_indices)
        # neigh_fea = index_points(l_enc_features.permute(0, 2, 1).contiguous(), neigh_indices)
        # attn = torch.einsum('bdn,bnkd->bnk', enc_features, neigh_fea) / np.sqrt(enc_features.size(1))
        # attn = torch.softmax(attn, dim=-1)
        # agg_features = (torch.einsum('bnk,bnkd->bdn', attn, neigh_fea) + enc_features) / 2
        # query_outputs = self.vote_query_generator(enc_xyz, agg_features.contiguous())
...
def detector(args, dataset_config):
    cfg = ModelConfig(args, dataset_config)
